...
import {
  ...
  sha384,
  sha512
} from "./chunk-5YRPZTNF.js";
import "./chunk-4ENIF2JH.js";
import {
  formatAbiItem,
  parseAbi,
  parseAbiItem
} from "./chunk-ALOQGI7K.js";
import {
  __export
...
function toPackedUserOperation(userOperation) {
  ...
  const paymasterAndData = paymaster ? concat2([
    paymaster,
  ...
  const [callData, factory, fees, nonce, authorization] = await Promise.all([
    ...
    (async () => {
      ...
      if (account2.entryPoint.version === "0.6")
        return {
          initCode: factory2 && factoryData ? concat2([factory2, factoryData]) : void 0
        };
      return {
        factory: factory2,
        factoryData
...
function toPacked(userOperation) {
  ...
  const accountGasLimits = concat(padLeft(fromNumber(verificationGasLimit || 0n), 16), padLeft(fromNumber(callGasLimit || 0n), 16));
  const initCode = toInitCode(userOperation);
  const gasFees = concat(padLeft(fromNumber(maxPriorityFeePerGas || 0n), 16), padLeft(fromNumber(maxFeePerGas || 0n), 16));
  const paymasterAndData = paymaster ? concat(paymaster, padLeft(fromNumber(paymasterVerificationGasLimit || 0n), 16), padLeft(fromNumber(paymasterPostOpGasLimit || 0n), 16), paymasterData || "0x") : "0x";
  const preVerificationGas = userOperation.preVerificationGas ?? 0n;
  return {
    accountGasLimits,
    callData,
...
function isEqual(hexA, hexB) {
  return equalBytes(fromHex4(hexA), fromHex4(hexB));
}
...
function toBytes4(hex, options = {}) {
  return fromHex4(hex, options);
}
...
function sha2565(value, options = {}) {
  const { as = typeof value === "string" ? "Hex" : "Bytes" } = options;
  const bytes = sha2564(from6(value));
  if (as === "Bytes")
    return bytes;
  return fromBytes2(bytes);
}
...
function fromBytes3(publicKey) {
  return fromHex5(fromBytes2(publicKey));
}
function fromHex5(publicKey) {
...
function toBytes5(publicKey, options = {}) {
  return fromHex4(toHex4(publicKey, options));
}
  ...
  constructor({ publicKey }) {
    super(`Value \`${publicKey}\` is an invalid public key size.`, {
      metaMessages: [
        ...
        `Received ${size4(from5(publicKey))} bytes.`
      ]
    });
    Object.defineProperty(this, "name", {
      enumerable: true,
...
function checksum2(address) {
  ...
  const hash = keccak2563(fromString4(hexAddress), { as: "Bytes" });
...
var SHA2562 = class extends HashMD2 {
  ...
  process(view, offset) {
    ...
    for (let i2 = 0; i2 < 64; i2++) {
      ...
      const T12 = H + sigma1 + Chi2(E, F, G) + SHA256_K2[i2] + SHA256_W2[i2] | 0;
...
var sha2566 = createHasher3(() => new SHA2562());
var sha2242 = createHasher3(() => new SHA2242());
var sha5123 = createHasher3(() => new SHA5122());
var sha3843 = createHasher3(() => new SHA3842());
...
function ecdh(Point2, ecdhOpts = {}) {
  ...
  function getSharedSecret(secretKeyA, publicKeyB, isCompressed = true) {
    ...
    const b2 = Point2.fromHex(publicKeyB);
...
var secp256k12 = createCurve2({ ...secp256k1_CURVE, Fp: Fpk1, lowS: true, endo: secp256k1_ENDO }, sha2566);
var TAGGED_HASH_PREFIXES = {};
function taggedHash(tag, ...messages) {
  let tagP = TAGGED_HASH_PREFIXES[tag];
  if (tagP === void 0) {
    const tagH = sha2566(utf8ToBytes2(tag));
    tagP = concatBytes(tagH, tagH);
    TAGGED_HASH_PREFIXES[tag] = tagP;
  }
  return sha2566(concatBytes(tagP, ...messages));
}
...
var secp256k1_hasher = (() => createHasher4(secp256k12.Point, (scalars) => {
...
function fromDerBytes(signature) {
  return fromDerHex(fromBytes2(signature));
}
function fromDerHex(signature) {
  const { r, s } = secp256k12.Signature.fromDER(from5(signature).slice(2));
  return { r, s };
}
...
function toBytes6(signature) {
  return fromHex4(toHex5(signature));
}
  ...
  constructor({ signature }) {
    super(`Value \`${signature}\` is an invalid signature size.`, {
      metaMessages: [
        ...
        `Received ${size4(from5(signature))} bytes.`
      ]
    });
    Object.defineProperty(this, "name", {
      enumerable: true,
...
var integerToCharacter2 = Object.fromEntries(Array.from("ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789+/").map((a, i2) => [i2, a.charCodeAt(0)]));
...
function fromBytes5(value, options = {}) {
  ...
  for (let i2 = 0, j = 0; j < value.length; i2 += 4, j += 3) {
    ...
    encoded[i2] = integerToCharacter2[y2 >> 18];
    encoded[i2 + 1] = integerToCharacter2[y2 >> 12 & 63];
    encoded[i2 + 2] = integerToCharacter2[y2 >> 6 & 63];
    encoded[i2 + 3] = integerToCharacter2[y2 & 63];
  }
  const k = value.length % 3;
  const end = Math.floor(value.length / 3) * 4 + (k && k + 1);
  let base64 = decoder3.decode(new Uint8Array(encoded.buffer, 0, end));
  if (pad4 && k === 1)
    base64 += "==";
  if (pad4 && k === 2)
    base64 += "=";
...
function fromHex8(value, options = {}) {
  return fromBytes5(fromHex4(value), options);
}
function toBytes7(value) {
  const base64 = value.replace(/=+$/, "");
  const size5 = base64.length;
  const decoded = new Uint8Array(size5 + 3);
  encoder4.encodeInto(base64 + "===", decoded);
  for (let i2 = 0, j = 0; i2 < base64.length; i2 += 4, j += 3) {
    const x = (characterToInteger2[decoded[i2]] << 18) + (characterToInteger2[decoded[i2 + 1]] << 12) + (characterToInteger2[decoded[i2 + 2]] << 6) + characterToInteger2[decoded[i2 + 3]];
    decoded[j] = x >> 16;
    decoded[j + 1] = x >> 8 & 255;
    decoded[j + 2] = x & 255;
  }
...
function bls(CURVE) {
  ...
  const G1 = Object.assign(G1_, createHasher4(G1_.Point, CURVE.G1.mapToCurve, {
    ...CURVE.htfDefaults,
    ...CURVE.G1.htfDefaults
  }));
  const G2_ = weierstrassPoints(CURVE.G2);
  const G2 = Object.assign(G2_, createHasher4(G2_.Point, CURVE.G2.mapToCurve, {
    ...CURVE.htfDefaults,
...
function signatureG1FromBytes(hex) {
  ...
  if (y2 * _2n7 / P !== aflag)
    y2 = Fp.neg(y2);
  const point = Point2.fromAffine({ x, y: y2 });
  point.assertValidity();
  return point;
}
...
function pointG2FromBytes(bytes) {
  ...
  const slc = (b2, from15, to) => bytesToNumberBE(b2.slice(from15, to));
...
function signatureG2FromBytes(hex) {
  ...
  if (isGreater || is0)
    y3 = Fp2.neg(y3);
  const point = Point2.fromAffine({ x, y: y3 });
  point.assertValidity();
  return point;
}
var bls12_381 = bls({
...
var Fp2562 = Field2(p256_CURVE2.p);
var Fp3842 = Field2(p384_CURVE2.p);
var Fp5212 = Field2(p521_CURVE2.p);
function createSWU2(Point2, opts) {
  const map = mapToCurveSimpleSWU2(Point2.Fp, opts);
  return (scalars) => map(scalars[0]);
}
var p2563 = createCurve2({ ...p256_CURVE2, Fp: Fp2562, lowS: false }, sha2566);
var p256_hasher2 = (() => {
  return createHasher4(p2563.Point, createSWU2(p2563.Point, {
    A: p256_CURVE2.a,
    B: p256_CURVE2.b,
...
var p3842 = createCurve2({ ...p384_CURVE2, Fp: Fp3842, lowS: false }, sha3843);
var p384_hasher2 = (() => {
  return createHasher4(p3842.Point, createSWU2(p3842.Point, {
    A: p384_CURVE2.a,
    B: p384_CURVE2.b,
...
var p5212 = createCurve2({ ...p521_CURVE2, Fp: Fp5212, lowS: false, allowedPrivateKeyLengths: [130, 131, 132] }, sha5123);
var p521_hasher2 = (() => {
  return createHasher4(p5212.Point, createSWU2(p5212.Point, {
    A: p521_CURVE2.a,
    B: p521_CURVE2.b,
...
function verify2(options) {
  ...
  return secp256r12.verify(signature, payload instanceof Uint8Array ? payload : fromHex4(payload), toHex4(publicKey).substring(2), ...hash ? [{ prehash: true, lowS: true }] : []);
}
...
function parseAsn1Signature2(bytes) {
  const r_start = bytes[4] === 0 ? 5 : 4;
  const r_end = r_start + 32;
  const s_start = bytes[r_end + 2] === 0 ? r_end + 3 : r_end + 2;
  const r = BigInt(fromBytes2(bytes.slice(r_start, r_end)));
  const s = BigInt(fromBytes2(bytes.slice(s_start)));
  return {
    r,
    s: s > p2564.CURVE.n / 2n ? p2564.CURVE.n - s : s
  };
}
...
function getAuthenticatorData(options = {}) {
  ...
  const rpIdHash = sha2565(fromString3(rpId));
...
function getClientDataJSON(options) {
  ...
  return JSON.stringify({
    type: "webauthn.get",
    challenge: fromHex8(challenge2, { url: true, pad: false }),
    origin,
    crossOrigin,
    ...extraClientData
  });
...
function getCredentialCreationOptions(options) {
  ...
  }, challenge: challenge2 = createChallenge2, excludeCredentialIds, name: name_, rp = {
...
async function sign2(options) {
  ...
  try {
    ...
    return {
      metadata: {
        authenticatorData: fromBytes2(new Uint8Array(response.authenticatorData)),
        clientDataJSON,
        challengeIndex,
        typeIndex,
...
function verify3(options) {
  ...
  const authenticatorDataBytes = fromHex4(authenticatorData);
  ...
  if (fromBytes2(toBytes7(challenge_extracted)) !== challenge2)
    return false;
  const clientDataJSONHash = sha2565(fromString4(clientDataJSON), {
    as: "Bytes"
  });
  const payload = concat5(authenticatorDataBytes, clientDataJSONHash);
  return verify2({
    hash,
    payload,
    publicKey,
...
var CredentialCreationFailedError2 = class extends BaseError3 {
  constructor({ cause } = {}) {
...
async function sign3(options) {
  ...
  const r = toBigInt2(slice4(signature_bytes, 0, 32));
  let s = toBigInt2(slice4(signature_bytes, 32, 64));
  if (s > p2564.CURVE.n / 2n)
    s = p2564.CURVE.n - s;
  return { r, s };
}
async function verify4(options) {
  ...
  return await globalThis.crypto.subtle.verify({
    name: "ECDSA",
    hash: "SHA-256"
  }, publicKey, concat5(fromNumber3(signature.r), fromNumber3(signature.s)), from6(payload));
}
...
function createWalletSendCallsRequest({ calls, from: from15, chainId, capabilities }) {
...
async function createSmartAccount(parameters) {
  ...
  return toSmartAccount({
    ...
    async sign(parameters2) {
      ...
      const signature = await sign4({ hash, owner });
      return wrapSignature({
        ownerIndex,
        signature
      });
...
async function sign4({ hash, owner }) {
  if (owner.type === "webAuthn") {
...
*/
//# sourceMappingURL=dist-TIUJJW2I.js.map
