...
contract CVTCTransferBasic is Ownable(msg.sender), ReentrancyGuard {
    IERC20 public cvtcToken;

    // Configuration simple pour les transferts échelonnés
    ...
    uint256 public totalImmediateTransfers;
    ...
    // Fonction principale de transfert - SANS AUCUNE LOGIQUE PREMIUM
    function transfer(address receiver, uint256 amount) external nonReentrant {
        ...
        if (amount < STAGGERED_THRESHOLD) {
            ...
            totalImmediateTransfers++;
            emit ImmediateTransfer(msg.sender, receiver, amount);
        } else {
            // Distribution échelonnée pour les montants >= 1000 CVTC
            uint256[] memory releaseSchedule = _calculateStaggeredSchedule(amount);
    ...
    // Fonction interne pour calculer la distribution échelonnée (1, 2, 4, 8, 16, 32...)
    function _calculateStaggeredSchedule(uint256 totalAmount) internal pure returns (uint256[] memory) {
    ...
    function getTransferStats() external view returns (
        ...
        uint256 totalStaggered,
        uint256 activeStaggered
    ) {
