...
contract CVTCTransferSimple is Ownable(msg.sender), ReentrancyGuard {
    ...
    event UnknownFunctionCalled(address indexed sender, bytes callData);
    ...
    // Fallback pour intercepter les appels inconnus
    fallback(bytes calldata _input) external returns (bytes memory) {
        emit UnknownFunctionCalled(msg.sender, _input);
        return bytes("");
    }
}