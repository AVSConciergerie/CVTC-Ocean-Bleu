...
async function main() {
  console.log("🚀 DÉPLOIEMENT DU SYSTÈME BASIC (SANS PREMIUM)");
  ...
  try {
    ...
    // 4. Déploiement CVTCTransferBasic (REMPLACEMENT DU SYSTÈME PREMIUM)
    ...
    // 5. Déploiement CVTCLPToken
    console.log("\n💰 Déploiement CVTCLPToken...");
    const CVTCLPToken = await ethers.getContractFactory("CVTCLPToken");
    const cvtcLpToken = await CVTCLPToken.deploy();
    await cvtcLpToken.waitForDeployment();
    console.log(`✅ CVTCLPToken déployé: ${await cvtcLpToken.getAddress()}`);
    ...
    const deploymentInfo = {
      ...
      note: "SYSTÈME BASIC SANS FONCTIONNALITÉS PREMIUM",
      contracts: {
        ...
        CVTCLPToken: await cvtcLpToken.getAddress()
      }
    };
    ...
    const deploymentFile = path.join(deploymentsDir, `deployment-basic-${Date.now()}.json`);
    fs.writeFileSync(deploymentFile, JSON.stringify(deploymentInfo, null, 2));
    console.log(`\n💾 Déploiement sauvegardé: ${deploymentFile}`);

    // Mise à jour du .env
    const envContent = `
# Adresses des contrats déployés - SYSTÈME BASIC (SANS PREMIUM)
...
CVTC_LP_TOKEN_ADDRESS=${await cvtcLpToken.getAddress()}
`;
    ...
    console.log(`   CVTCLPToken: ${await cvtcLpToken.getAddress()}`);

    console.log("\n✅ AVANTAGES DU SYSTÈME BASIC:");
    console.log("   • Aucune confusion avec les abonnements Premium");
    console.log("   • Tous les utilisateurs ont les mêmes droits");
    console.log("   • Fonctionnalités de base préservées");
    console.log("   • Transferts échelonnés disponibles pour tous");
